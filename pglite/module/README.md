...
- **Roles.** PGlite starts as one superuser with no app roles; create any role used via `db.setContext({ role })` through `pglite.extensionSql` (see `__tests__/basic.test.ts` for the pattern).
